...
require(sp) # for SpatialPoints class
...
### positive records
sb_pos <- read.csv("Data/solentbirds-pos-3May2017.csv")
# remove unneeded columns for compatibility
sb_pos$BTO.code <- NULL
sb_pos$Colour.Markings <- NULL

### negative records
sb_neg <- read.csv("Data/solentbirds-neg-3May2017.csv")
...
source('load_hiwwt.R')
hiwwt$sp[which(hiwwt$sp %in% webspplist$sp ==F)]
# remove any species we don't need
hiwwt <- subset(hiwwt, sp %in% 'Sand Martin' == F)
