...
digisnap <- readOGR(dsn='../../Solent Bird Studies GIS Data/Survey results', 
                    layer='all_snapshots') %>%
  spTransform(stratcode@proj4string) # convert digisnap to have same coord system as stratcode
...
sum(is.na(snap$Easting)) # 98 records lack coordinates still
# some have the strat code recorded - so can add polygon midpoints as coords



# extract the sitecodes
sites <- unique(stratcode$Site_code) %>% as.character
...
sum(is.na(snap$Easting) & !(is.na(snap$location))) # 77 extra records :)
...
mat <- match(snap$location, stratcode$Site_code)
...
sum(is.na(snap$Easting)) # 20 records - these all are from before protocol changed
