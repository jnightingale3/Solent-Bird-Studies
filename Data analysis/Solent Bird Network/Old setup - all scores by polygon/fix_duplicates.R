...
daymaxspp <- ddply(solent_pdata, .(Recorder, Site_ID, Date, Species), function(x) {
...
daytotalrec <- ddply(daymaxspp, .(Recorder, Site_ID, Date), function(x) {
  data.frame(Total = sum(x$Max, na.rm=T))
})
# then include only the highest assemblage per day
daytotal <- ddply(daytotalrec, .(Site_ID, Date), function(x) {
  data.frame(Total = max(x$Total, na.rm=T))
})


## each site's highest count of each species
sitemaxspp <- ddply(daymaxspp, .(Site_ID, Species), function(x) {
