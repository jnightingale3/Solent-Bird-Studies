...
require(forcats)  # for fct_collapse() to combine factor levels
require(Hmisc)    # for capitalize()
...
birds_visitinfo$Protect %<>% fct_collapse(
  None = c('none', 'None', 'null'),
  Banger = c('Banger', 'propane banger'),
  Rope = c('rope', 'Ropes', 'string', 'streamers', 'Streamers'),
  Rope_and_banger = c('Propane banger and string', 'string and bangers', 'String and propane banger', 'Ropes and bangers'),
  Scarecrow = c('scarecrow', 'Scarecrow')
) %>% droplevels

# Habitat
birds_visitinfo$Habitat[which(birds_visitinfo$Habitat=='')] <- NA
birds_visitinfo$Habitat %<>% fct_collapse(
  Amenity = levels(birds_visitinfo$Habitat)[grep('Amenity', levels(birds_visitinfo$Habitat))],
  Arable = levels(birds_visitinfo$Habitat)[grep('Arable', levels(birds_visitinfo$Habitat))],
  Grazing = c('Coastal grazing marsh', 'Horse grazing', 'Pasture', 'Sheep grazing', 'common land'),
  Urban = c('Residential', 'Commercial')
) %>% droplevels
 

# Disturbance
birds_visitinfo$Disturb[which(birds_visitinfo$Disturb == '')] <- NA
birds_visitinfo$Disturb %<>% as.character %>% capitalize %>% factor %>% droplevels

birds_visitinfo$Disturb %<>% fct_collapse(
  Traffic = c('Cars', 'Vehicle traffic'),
  Dog = levels(birds_visitinfo$Disturb)[grep('Dog', levels(birds_visitinfo$Disturb))],
  Construction = c('Construction site', 'Construction work', 'Digger machine'),
  Livestock = c('Cattle', levels(birds_visitinfo$Disturb)[grep('Horse', levels(birds_visitinfo$Disturb))]),
  Shooting= c('Farmer shotgun', 'Shooting'),
  None = c('None', 'Null'),
  Amenity = c('Cyclist', 'Jogger', 'Sport/amenity', 'Walker', 'Adjacent to golf course'),
  Aircraft = c('Low flying aircraft', 'Microlight'),
  Watercraft = c('Hovercraft', 'Small fishing boats',levels(birds_visitinfo$Disturb)[grep('Kayak', levels(birds_visitinfo$Disturb))]),
  Farm_operation = c('Farm/ grounds maintenence', 'Farm operation')
) %>% droplevels
## WTF is 'Cheese' ??!!?!?!
...
# save arable sub-categories in separate vector for possible analysis?
cleanarableinfo <- subset(clean_visitinfo, select=c('visitid', 'Habitat'))
cleanarableinfo$Habitat[!grepl('Arable', clean_visitinfo$Habitat)] <- NA
cleanarableinfo %<>% droplevels
summary(cleanarableinfo)

clean_visitinfo$Habitat %<>% fct_collapse(
  Amenity = c('Amenity Grassland', 'Golf course', 'Playing field'),
  Arable = levels(clean_visitinfo$Habitat)[grep('Arable', levels(clean_visitinfo$Habitat))],
  Beach = 'Sand/shingle bank',
...
clean_visitinfo$Disturb %<>% as.character %>% capitalize %>% factor %>% droplevels

clean_visitinfo$Disturb %<>% fct_collapse(
  Dog = levels(clean_visitinfo$Disturb)[grep('Dog', levels(clean_visitinfo$Disturb))],
  Livestock = levels(clean_visitinfo$Disturb)[grep('Horse', levels(clean_visitinfo$Disturb))],
  Amenity = c('Cyclist', 'Jogger', 'Sport/amenity', 'Walker'),
  Aircraft = c('Low flying aircraft'),
  Watercraft = c('Kite surfing', 'Motor boat', 'Sailing Boat', levels(clean_visitinfo$Disturb)[grep('Kayak', levels(clean_visitinfo$Disturb))]),
  Farm_operation = c('Farm/grounds maintenance')
) %>% droplevels
