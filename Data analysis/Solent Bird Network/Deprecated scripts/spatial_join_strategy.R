...
## read in data
polys <- readOGR(dsn='../Solent Bird Studies GIS Data',
                 layer='Waders & Brent Geese Sites_region')
# plot(polys) # looks ok
...
is.na(solent_filter$Easting)%>%sum # yes, but only 11 lack them
solent_filter <- solent_filter[!is.na(solent_filter$Easting),]
